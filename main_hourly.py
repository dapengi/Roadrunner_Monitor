...
Uses Granite Speech pipeline for high-accuracy transcription and uploads to Seafile
...
def process_entry(entry, proxy_manager, seafile_client):
    ...
    try:
        ...
        # Create and upload manifest to Seafile, then send webhook
        manifest_uploaded = False
        if seafile_results:
            try:
                manifest = create_manifest(
                    committee=filename_info['committee'],
                    meeting_date=filename_info['date'],
                    base_name=base_name,
                ...
                if seafile_client.write_file(manifest_path, manifest_content):
                    logger.info(f"  ✅ Uploaded manifest to Seafile: {manifest_path}")
                    ...
                    if webhook_success:
                        logger.info("  ✅ Webhook sent to n8n")
                    else:
                        logger.warning("  ⚠️ Webhook to n8n failed (non-critical)")
                else:
                    logger.warning(f"  ⚠️ Failed to upload manifest to Seafile")

            except Exception as e:
                logger.error(f"  ❌ Error creating/uploading manifest: {e}", exc_info=True)
