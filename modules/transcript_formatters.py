...
class TranscriptFormatter:
    ...
    def convert_speaker_labels(self, segments: List[Dict]) -> List[Dict]:
        """
        Convert speaker labels from 'Speaker A', 'Speaker B' format to 'speaker_0', 'speaker_1'.

        Args:
            segments: List of segments with speaker labels

        Returns:
            Updated segments with numeric speaker IDs
        """
        speaker_mapping = {}
        next_speaker_id = 0
        ...
        if audio_events:
            for event in audio_events:
                words.append({
                    ...
                    "start": event['start'],
                    "end": event['end'],
                    "speaker_id": event.get('speaker_id', 'speaker_0')
                })
    ...
    def to_csv(self, segments: List[Dict]) -> str:
        ...
        00:00:05,speaker_0,"Hello everyone..."
        ...
        for segment in segments:
            ...
            speaker = segment.get('speaker_id', 'speaker_0')
            text = segment.get('text', '').strip()
    ...
    def to_txt(self, segments: List[Dict]) -> str:
        ...
        Format:
        00:00:05 | speaker_0 | Hello everyone, welcome to today's committee hearing.
        00:00:18 | speaker_1 | Thank you, Mr. Chairman.
