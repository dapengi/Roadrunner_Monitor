#!/usr/bin/env python3
"""
Voice Enrollment using Sherpa-ONNX Diarization + Pyannote Embeddings
Hybrid approach: Sherpa for speaker detection, Pyannote for voice prints
"""
...
# Apply torchaudio patches BEFORE any imports that use it
import torchaudio
...
# Import Sherpa diarization (already working on your system)
from modules.sherpa_diarization import SherpaDiarizer

# Import Pyannote for embeddings only (after patches applied)
from pyannote.audio import Inference
from data.committee_rosters import COMMITTEE_ROSTERS


class HybridEnrollment:
    """Voice enrollment using Sherpa diarization + Pyannote embeddings."""
    ...
    def __init__(self, temp_dir: str = "voice_enrollment/temp"):
        ...
        # Load Pyannote for embeddings only
        print("Loading Pyannote embedding model...")
        self.embedding_model = Inference('pyannote/wespeaker-voxceleb-resnet34-LM')
    ...
    def generate_speaker_embeddings(self, diarization_result: Dict, samples_per_speaker: int = 10) -> Dict:
        """
        Generate Pyannote voice embeddings for each detected speaker.
        ...
        print(f"\n🔊 Generating Pyannote embeddings for {len(speaker_segments)} speakers...")
    ...
    def _generate_embedding(self, audio_file: str, start_time: float, end_time: float) -> Optional[np.ndarray]:
        """Generate Pyannote embedding for audio segment."""
        try:
            import librosa
            ...
            # Generate embedding
            audio_dict = {'waveform': segment, 'sample_rate': sr}
    ...
    def save_voice_database(self, labeled_speakers: Dict, meeting_file: str, output_file: str):
        ...
        voice_db = {
            ...
            'description': 'Voice embeddings database from Sherpa diarization + Pyannote embeddings',
        ...
        with open(summary_file, 'w') as f:
            ...
            f.write(f"Method: Sherpa-ONNX diarization + Pyannote embeddings\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Enroll voices using Sherpa diarization + Pyannote embeddings'
    )
    parser.add_argument(
        'audio_file',
    ...
    print("\n" + "=" * 80)
    print("🎙️  VOICE ENROLLMENT - SHERPA + PYANNOTE HYBRID")
    print("=" * 80)
    print(f"Audio file: {args.audio_file}")
    print(f"Committee: {args.committee}")
    print(f"Method: Sherpa-ONNX diarization + Pyannote embeddings")
    print("=" * 80)

    # Initialize enrollment
    try:
        enrollment = HybridEnrollment()
    except Exception as e:
    ...
    # Step 2: Generate Pyannote embeddings
    print("\n📍 STEP 2: Generate Voice Embeddings (Pyannote)")
